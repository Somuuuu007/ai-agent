...
export default function AIAgentPage() {
  ...
  useEffect(() => {
    ...
    if (isMobile) {
      // On mobile: stack vertically (rows), left takes auto height, right takes remaining
      if (gridRef.current) {
        gsap.to(gridRef.current, {
          duration,
          ease,
          gridTemplateColumns: '1fr',
          gridTemplateRows: showOutput ? 'auto 1fr' : '1fr 0fr',
        })
      }
      if (rightRef.current) {
  ...
  const outputPanel = getDisplayContent() && (
                                ...
                                <script>
                                  document.addEventListener('DOMContentLoaded', function() {
                                    document.addEventListener('click', function(e) {
                                      ...
                                      if (target && target.href) { e.preventDefault(); window.scrollTo(0, 0); return false; }
                                    });
                                  });
                                </script>
                              </head>
  ...
  return (
    <div className={`bg-black ${isMobile ? 'min-h-screen overflow-y-auto' : 'h-screen overflow-hidden'}`}>
      ...
      <div
        ...
        style={{ gridTemplateColumns: '1fr 0fr', gridTemplateRows: '1fr' }}
      >
        <div ref={leftRef} className="flex flex-col overflow-hidden relative z-10" style={{ height: isMobile ? 'auto' : '100vh', minHeight: isMobile ? '50vh' : undefined }}>
          ...
          {!lastPrompt ? (
            ...
            <>
              ...
              {/* On mobile: render output panel here, between button and input */}
              {isMobile && showOutput && (
                <div className="flex-shrink-0 mx-4 mb-2" style={{ height: '80vh' }}>
                  {outputPanel}
                </div>
              )}
        ...
        {/* Desktop only: right panel as grid sibling */}
        {!isMobile && (
          <div ref={rightRef} className="opacity-0 min-w-0 flex flex-col overflow-hidden relative z-10" style={{ height: 'calc(100vh - 2rem)' }}>
            {outputPanel}
          </div>
        )}
      </div>
